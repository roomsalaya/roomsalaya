...
const BankTransferForm316: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "invoices316"));
    ...
    const handleSubmitProof = async () => {
        ...
        try {
            ...
            const imageRef = ref(storage, `proofs316/${Date.now()}_${Math.random()}`);
            ...
            await addDoc(collection(db, "paymentProofs316"), {
            ...
            navigate('/paymenthistory316', { state: { selectedText, selectedImage: downloadURL } }); // ปรับเส้นทางการนำทาง
        } catch (error) {
    ...
    const handleViewHistory = () => {
        navigate('/paymenthistory316'); // นำทางไปยังหน้าประวัติการชำระเงิน
    };
    ...
    return (
        <div className='bank-container'>
            <h3>
                แจ้งชำระค่าเช่า 316
                <AppMenu316 /> {/* อัปเดตเพื่อสะท้อนคอมโพเนนต์ใหม่ */}
            </h3>
...
export default BankTransferForm316;
