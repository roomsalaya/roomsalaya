...
const HouseInfo332 = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                const invoiceRef = doc(db, "invoices332", editingId);
                await updateDoc(invoiceRef, invoiceData);
                await fetchInvoices();
                setEditingId(null);
            } else {
                // Add new invoice
                await addDoc(collection(db, "invoices332"), invoiceData);
                await fetchInvoices();
            }
    ...
    const toggleStatus = async (invoice: Invoice) => {
        try {
            const invoiceRef = doc(db, "invoices332", invoice.id!);
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteDoc(doc(db, "invoices332", id));
