...
const Allowpayment205: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const querySnapshot = await getDocs(collection(db, "paymentProofs205")); // Adjust collection name
    ...
    const handleStatusChange = async (key: string, currentStatus: string) => {
        ...
        const docRef = doc(db, "paymentProofs205", key); // Adjust collection name
...
export default Allowpayment205;
