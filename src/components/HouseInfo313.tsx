...
const HouseInfo313 = () => {
    ...
    const fetchInvoices = async () => {
        try {
            const querySnapshot = await getDocs(collection(db, "invoices313")); // ใช้ชื่อ collection ที่ต่างกัน
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                const invoiceRef = doc(db, "invoices313", editingId); // ใช้ชื่อ collection ที่ต่างกัน
                await updateDoc(invoiceRef, invoiceData);
                await fetchInvoices();
                setEditingId(null);
            } else {
                // เพิ่มใบแจ้งหนี้ใหม่
                await addDoc(collection(db, "invoices313"), invoiceData); // ใช้ชื่อ collection ที่ต่างกัน
                await fetchInvoices();
            }
    ...
    const toggleStatus = async (invoice: Invoice) => {
        try {
            const invoiceRef = doc(db, "invoices313", invoice.id!); // ใช้ชื่อ collection ที่ต่างกัน
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteDoc(doc(db, "invoices313", id)); // ใช้ชื่อ collection ที่ต่างกัน
