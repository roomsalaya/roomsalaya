...
const BankTransferForm331: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "invoices331"));
    ...
    const handleSubmitProof = async () => {
        ...
        try {
            ...
            const imageRef = ref(storage, `proofs331/${Date.now()}_${Math.random()}`);
            ...
            await addDoc(collection(db, "paymentProofs331"), {
            ...
            navigate('/paymenthistory331', { state: { selectedText, selectedImage: downloadURL } }); // ปรับเส้นทางการนำทาง
        } catch (error) {
    ...
    const handleViewHistory = () => {
        navigate('/paymenthistory331'); // นำทางไปยังหน้าประวัติการชำระเงิน
    };
    ...
    return (
        <div className='bank-container'>
            <h3>
                แจ้งชำระค่าเช่า 331
                <AppMenu331 /> {/* อัปเดตเพื่อสะท้อนคอมโพเนนต์ใหม่ */}
            </h3>
...
export default BankTransferForm331;
