...
const Allowpayment329: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const querySnapshot = await getDocs(collection(db, "paymentProofs329")); // Adjust collection name
    ...
    const handleStatusChange = async (key: string, currentStatus: string) => {
        ...
        const docRef = doc(db, "paymentProofs329", key); // Adjust collection name
...
export default Allowpayment329;
