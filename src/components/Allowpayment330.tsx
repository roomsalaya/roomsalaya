...
const Allowpayment330: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const querySnapshot = await getDocs(collection(db, "paymentProofs330")); // Adjust collection name
    ...
    const handleStatusChange = async (key: string, currentStatus: string) => {
        ...
        const docRef = doc(db, "paymentProofs330", key); // Adjust collection name
...
export default Allowpayment330;
