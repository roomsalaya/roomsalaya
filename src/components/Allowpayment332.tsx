...
const Allowpayment332: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const querySnapshot = await getDocs(collection(db, "paymentProofs332")); // Adjust collection name
    ...
    const handleStatusChange = async (key: string, currentStatus: string) => {
        ...
        const docRef = doc(db, "paymentProofs332", key); // Adjust collection name
...
export default Allowpayment332;
