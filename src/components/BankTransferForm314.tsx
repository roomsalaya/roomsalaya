...
const BankTransferForm314: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "invoices314"));
    ...
    const handleSubmitProof = async () => {
        ...
        try {
            ...
            const imageRef = ref(storage, `proofs314/${Date.now()}_${Math.random()}`);
            ...
            await addDoc(collection(db, "paymentProofs314"), {
            ...
            navigate('/paymenthistory314', { state: { selectedText, selectedImage: downloadURL } }); // ปรับเส้นทางการนำทาง
        } catch (error) {
    ...
    const handleViewHistory = () => {
        navigate('/paymenthistory314'); // นำทางไปยังหน้าประวัติการชำระเงิน
    };
    ...
    return (
        <div className='bank-container'>
            <h3>
                แจ้งชำระค่าเช่า 314
                <AppMenu314 /> {/* อัปเดตเพื่อสะท้อนคอมโพเนนต์ใหม่ */}
            </h3>
...
export default BankTransferForm314;
