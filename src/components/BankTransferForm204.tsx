...
const BankTransferForm204: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "invoices204"));
    ...
    const handleSubmitProof = async () => {
        ...
        try {
            ...
            const imageRef = ref(storage, `proofs204/${Date.now()}_${Math.random()}`);
            ...
            await addDoc(collection(db, "paymentProofs204"), {
            ...
            navigate('/paymenthistory204', { state: { selectedText, selectedImage: downloadURL } }); // ปรับเส้นทางการนำทาง
        } catch (error) {
    ...
    const handleViewHistory = () => {
        navigate('/paymenthistory204'); // นำทางไปยังหน้าประวัติการชำระเงิน
    };
    ...
    return (
        <div className='bank-container'>
            <h3>
                แจ้งชำระค่าเช่า 204
                <AppMenu204 /> {/* อัปเดตเพื่อสะท้อนคอมโพเนนต์ใหม่ */}
            </h3>
...
export default BankTransferForm204;
