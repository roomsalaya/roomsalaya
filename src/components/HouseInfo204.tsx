...
const HouseInfo204 = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                const invoiceRef = doc(db, "invoices204", editingId);
                await updateDoc(invoiceRef, invoiceData);
                await fetchInvoices();
                setEditingId(null);
            } else {
                // Add new invoice
                await addDoc(collection(db, "invoices204"), invoiceData);
                await fetchInvoices();
            }
    ...
    const toggleStatus = async (invoice: Invoice) => {
        try {
            const invoiceRef = doc(db, "invoices204", invoice.id!);
    ...
    const handleDelete = async (id: string) => {
        try {
            await deleteDoc(doc(db, "invoices204", id));
