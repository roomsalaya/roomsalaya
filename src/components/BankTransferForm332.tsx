...
const BankTransferForm332: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "invoices332"));
    ...
    const handleSubmitProof = async () => {
        ...
        try {
            ...
            const imageRef = ref(storage, `proofs332/${Date.now()}_${Math.random()}`);
            ...
            await addDoc(collection(db, "paymentProofs332"), {
            ...
            navigate('/paymenthistory332', { state: { selectedText, selectedImage: downloadURL } }); // ปรับเส้นทางการนำทาง
        } catch (error) {
    ...
    const handleViewHistory = () => {
        navigate('/paymenthistory332'); // นำทางไปยังหน้าประวัติการชำระเงิน
    };
    ...
    return (
        <div className='bank-container'>
            <h3>
                แจ้งชำระค่าเช่า 332
                <AppMenu332 /> {/* อัปเดตเพื่อสะท้อนคอมโพเนนต์ใหม่ */}
            </h3>
...
export default BankTransferForm332;
